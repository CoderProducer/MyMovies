...
<head>
    ...
    <script>
        function check() {
            var oInput=document.getElementsByName("shipin");
			...
			function alldel(){
				...
				var shipin=document.getElementsByName("shipin");//得到一组checkbox  相当于数组
				alert("id值："+shipin);
				//循环这一组checkbox让每一个checkbox选中
				for(var i=0;i<shipin.length;i++){
					if(shipin[i].checked==true){//代表选中
						arr.push(shipin[i].value);
						f=true;
					}
				}
				...
				if(f==true){
					if(confirm("您确认要删除吗？"))
						location.href="${pageContext.request.contextPath}/VideoControl/videoAdminDelete.do?videoIds="+arr;
				}else{
					alert("请至少选中一条进行批量删除");
				}
			}
...
<body >
<p></p>
<form class="form-inline definewidth m20" action="${pageContext.request.contextPath}/UserControl/f.do" method="post">
    <span style="color: #777777"><strong>视频名：</strong></span>
...
<table class="table table-bordered table-hover definewidth m10">
    ...
    <tr>
        ...
        <td style="text-align: center">视频名称</td>
        <td style="text-align: center">视频封面</td>
        <td style="text-align: center">视频大类</td>
        <td style="text-align: center">视频小类</td>
        <td style="text-align: center">时长</td>
        <td style="text-align: center">上传日期</td>
        <td style="text-align: center">视频简介</td>
    </tr>
   
    <c:forEach var="message" items="${videoAdminList}">
  		<tr>
  			<td style="width: 10%">
  				<input type="checkbox" style="text-align: center" name="shipin" value="${message.video_Id}" />
			</td>
			<td style="width: 10%;text-align: center">${message.video_Name}</td>
			<td style="width: 20%;text-align: center"><img src="${pageContext.request.contextPath}/static/images/m11.jpg" title="album-name" alt=" " width="120px" height="110px" /></td>
			<td style="width: 10%;text-align: center">${message.video_BigType}</td>
			<td style="width: 10%;text-align: center">${message.video_SmallType}</td>
			<td style="width: 10%;text-align: center">${message.video_Duration}</td>
			<td style="width: 10%;text-align: center"><df:formatDate value="${message.video_Published}" pattern="yyyy-MM-dd"></df:formatDate></td>	
			<td style="width: 20%;text-align: center">${message.video_Introduce}</td>		
		</tr>   					
	</c:forEach>
</table>
		...
		<div class="page_nav">
         ...
         <c:choose>
             <c:when test="${page.pageNow != 1 }"><!-- 如果当前页为1，则不显示首页和上一页 -->
                 <a href="${pageContext.request.contextPath}/VideoControl/videosManage.do?pageNow=1">首页</a> 
                 <a href="${pageContext.request.contextPath}/VideoControl/videosManage.do?pageNow=${page.pageNow-1 }">上一页</a>
             </c:when>
         </c:choose>
         <!-- 遍历页码 -->
         <c:forEach begin="${begin }" end="${end }" var="index">
             <c:choose>
                 ...
                 <c:otherwise><!-- 否则，普通显示 -->
                     <a id="index" href="${pageContext.request.contextPath}/VideoControl/videosManage.do?pageNow=${index }">${index }</a>
                 </c:otherwise>
             </c:choose>
         </c:forEach>
         <c:choose>
             <c:when test="${page.pageNow != page.totalPageCount }"><!-- 如果当前页为总的记录数，则不显示末页和下一页 -->
                 <a href="${pageContext.request.contextPath}/VideoControl/videosManage.do?pageNow=${page.pageNow+1 }">下一页</a> 
                 <a href="${pageContext.request.contextPath}/VideoControl/videosManage.do?pageNow=${page.totalPageCount }">末页</a>
             </c:when>
         </c:choose>
...
<script type="text/javascript">
             ...
             $("#pn_btn").click(function() {
                 ...
                 window.location = "${pageContext.request.contextPath}/VideoControl/videosManage.do?pageNow=" + pageNow;
             });
</script>
