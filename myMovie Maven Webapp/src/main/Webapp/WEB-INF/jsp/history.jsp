            ...
            <div class="browse-agile-w3ls general-w3ls">
				...
				<div class="container">               
                    <div class="w3ls-news-result">
                        <h4>搜索 结果 : <span>${hiscount }条</span></h4>
                    </div>
					<ul class="main-member">
					<c:forEach items="${hList }" var="hList2">
					<c:set value="${hList2.vList }" var="h"/>
						<c:forEach items="${h}" var="message">
						<li id="photoAddress" onclick="photoKick();">							 
                            ...
                            <p class="mem-num">${message.video_BigType}集数:&nbsp;第${message.video_Season}集</p>	
						  	...
						  	<p class="mem-num">上次观看时间:&nbsp;<df:formatDate value="${hList2.history_LastTime }" pattern="yyyy-MM-dd"></df:formatDate></p>	                            			  
					    </a>					    					  
					</li>
					</c:forEach>					
					</c:forEach>								
                 ...
                 <div class="clearfix"></div>
                  <c:set var="count" value="${hiscount}"></c:set>
                  <c:if test="${count<1}">
		            <ul class="info_box">
		                <li style="text-align: center;font-size: 50px;color: red">暂无观看记录！</li>	                
		            </ul>
		          </c:if>
	       </div>
	  </div>
