...
<head>
    ...
    <script>
        function check() {
            var oInput=document.getElementsByName("pinglun");
			...
			function alldel(){
				...
				var pinglun=document.getElementsByName("pinglun");//得到一组checkbox  相当于数组
				alert("id值："+pinglun);
				//循环这一组checkbox让每一个checkbox选中
				for(var i=0;i<pinglun.length;i++){
					if(pinglun[i].checked==true){//代表选中
						arr.push(pinglun[i].value);
						f=true;
					}
				}
				...
				if(f==true){
					if(confirm("您确认要删除吗？"))
						location.href="${pageContext.request.contextPath}/ViewControl/viewDelete.do?ids="+arr;
				}else{
					alert("请至少选中一条进行批量删除");
				}
			}
                            ...
                            <div class="desc" >
                                <ul style="list-style: none">
                                    ...
                                    <!-- <input type="submit" value="删除"/> -->
                                     <c:forEach var="viewlist2" items="${viewList}">
                                     <c:set value="${viewlist2.viewList }" var="h"/>
                                     <c:forEach var="message" items="${h}">
                                    <li>
                                        <input type="checkbox" style="float: left" name="pinglun" value="${viewlist2.view_Id}" /><br>
                                        <span style="float: left;color: red">
                                        <df:formatDate value="${message.video_Published}" pattern="yyyy-MM-dd HH:mm:ss"></df:formatDate>&nbsp;&nbsp;&nbsp;${message.video_Name}:&nbsp;&nbsp;&nbsp;</span>
				...
				<div class="page_nav">
		         ...
		         <c:choose>
		             <c:when test="${pages.pageNow != 1 }"><!-- 如果当前页为1，则不显示首页和上一页 -->
		                 <a href="${pageContext.request.contextPath}/ViewControl/pinglun.do?pageNow=1">首页</a> 
		                 <a href="${pageContext.request.contextPath}/ViewControl/pinglun.do?pageNow=${pages.pageNow-1 }">上一页</a>
		             </c:when>
		         </c:choose>
		         <!-- 遍历页码 -->
		         <c:forEach begin="${begin }" end="${end }" var="index">
		             <c:choose>
		                 ...
		                 <c:otherwise><!-- 否则，普通显示 -->
		                     <a href="${pageContext.request.contextPath}/ViewControl/pinglun.do?pageNow=${index }">${index }</a>
		                 </c:otherwise>
		             </c:choose>
		         </c:forEach>
		         <c:choose>
		             <c:when test="${pages.pageNow != pages.totalPageCount }"><!-- 如果当前页为总的记录数，则不显示末页和下一页 -->
		                 <a href="${pageContext.request.contextPath}/ViewControl/pinglun.do?pageNow=${pages.pageNow+1 }">下一页</a> 
		                 <a href="${pageContext.request.contextPath}/ViewControl/pinglun.do?pageNow=${pages.totalPageCount }">末页</a>
		             </c:when>
		         </c:choose>
...
<script type="text/javascript">
    ...
    <script type="text/javascript">
		             ...
		             $("#pn_btn").click(function() {
		                 ...
		                 window.location = "${pageContext.request.contextPath}/ViewControl/pinglun.do?pageNow=" + pageNow;
		             });
		</script>
